...
const AuthForm = () => {
  ...
  async function formSubmitHandler(event) {
    ...
    let url;
    if (isLogin) {
      url = 'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyCAHGCbTf-NoWVprySW0qYLHdhBGj5DPJk'

    } else {
      url = 'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyCAHGCbTf-NoWVprySW0qYLHdhBGj5DPJk'
    ...
    if (!isLogin) {
      try {
        const response = await axios.post('https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=AIzaSyCAHGCbTf-NoWVprySW0qYLHdhBGj5DPJk',
          {
